...
class EtfSiteBuilderTests(unittest.TestCase):
    ...
    def test_allocation_rows_includes_weight_change_1m_and_1y(self) -> None:
        ...
        cash_row = next(row for row in rows if row.market_id == "ie:XSTR")
        ...
        self.assertEqual(etf_row.recommended_ie_weight_pct, 95)
        self.assertEqual(cash_row.recommended_ie_weight_pct, 5)
    ...
    def test_build_index_html_writes_file(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn(">95%</td>", text)
            self.assertIn(">5%</td>", text)
            self.assertIn("XSTR", text)
            self.assertIn("allocated to XSTR", text)
